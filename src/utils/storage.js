...
export const getRewards = async () => {
  ...
  return profile?.rewards || { points: 0, badges: [], discounts: [], streak: { count: 0, lastRefillAt: null } };
};

export const addPoints = async (points) => {
  const profile = (await getUserProfile()) || {};
  const rewards = profile.rewards || { points: 0, badges: [], discounts: [], streak: { count: 0, lastRefillAt: null } };
  rewards.points += points;
  await saveUserProfile({ ...profile, rewards });
  await AsyncStorage.setItem(STORAGE_KEYS.REWARDS_STATE, JSON.stringify(rewards));
...
export const addDiscount = async (discount) => {
  const profile = (await getUserProfile()) || {};
  const rewards = profile.rewards || { points: 0, badges: [], discounts: [], streak: { count: 0, lastRefillAt: null } };
  rewards.discounts.push({ ...discount, used: false, grantedAt: new Date().toISOString() });
  await saveUserProfile({ ...profile, rewards });
  await AsyncStorage.setItem(STORAGE_KEYS.REWARDS_STATE, JSON.stringify(rewards));
...
export const redeemDiscount = async (discountId) => {
  const profile = (await getUserProfile()) || {};
  const rewards = profile.rewards || { points: 0, badges: [], discounts: [], streak: { count: 0, lastRefillAt: null } };
  const idx = (rewards.discounts || []).findIndex((d) => d.id === discountId && !d.used);
  if (idx >= 0) {
    rewards.discounts[idx] = { ...rewards.discounts[idx], used: true, usedAt: new Date().toISOString() };
    await saveUserProfile({ ...profile, rewards });
    await AsyncStorage.setItem(STORAGE_KEYS.REWARDS_STATE, JSON.stringify(rewards));
    return true;
  }
...
const daysBetween = (d1, d2) => Math.abs((new Date(d1).setHours(0,0,0,0) - new Date(d2).setHours(0,0,0,0)) / (1000*60*60*24));
...
const evaluateBadges = async (stats) => {
  ...
  if (rewards.streak?.lastRefillAt) {
    ...
    if (diff <= 7) rewards.streak.count = (rewards.streak.count || 0) + 1; else rewards.streak.count = 1;
  } else {
  ...
  const push = (id, name, icon) => {
    if (!rewards.badges.find((b) => b.id === id)) rewards.badges.push({ id, name, icon, unlockedAt: new Date().toISOString() });
  };
...
export const saveSelectedProducts = async (products) => {
  try {
    await AsyncStorage.setItem(STORAGE_KEYS.SELECTED_PRODUCTS, JSON.stringify(products));
    return true;
  } catch (error) {
...
export const getSelectedProducts = async () => {
  try {
    const products = await AsyncStorage.getItem(STORAGE_KEYS.SELECTED_PRODUCTS);
...
export const addRefillToHistory = async (refill) => {
  try {
    ...
    await AsyncStorage.setItem(STORAGE_KEYS.REFILL_HISTORY, JSON.stringify(history));
    ...
    if ((stats.totalRefills % 5) === 0) await addDiscount({ id: `DISC-${stats.totalRefills}`, label: `-${Math.min(5 + (stats.totalRefills/5), 20)}% sur votre prochaine recharge`, value: Math.min(5 + (stats.totalRefills/5), 20) });
...
export const updateImpactStats = async (products) => {
  try {
    ...
    const estimateSavings = (p) => {
      const regular = typeof p.priceRegular === 'number' ? p.priceRegular : 8.0; // €
      const refill = typeof p.priceRefill === 'number' ? p.priceRefill : regular * 0.7; // -30%
    ...
    await AsyncStorage.setItem(STORAGE_KEYS.IMPACT_STATS, JSON.stringify(newStats));
    return newStats;
  } catch (error) {
...
export const saveUserProfile = async (profile) => {
  try {
    ...
    const p = {
      ...profile,
      rewards: profile.rewards || { points: 0, badges: [], discounts: [], streak: { count: 0, lastRefillAt: null } },
    };
...
export const updateUserProfile = async (partial) => {
  ...
  const next = { ...current, ...partial, rewards: current.rewards || { points: 0, badges: [], discounts: [], streak: { count: 0, lastRefillAt: null } } };
