...
export const getRecommendedProducts = (answers) => {
  ...
  if (answers && answers._beautyProfile) {
    ...
    const aiNeeds = Array.isArray(p.ai?.needs) ? p.ai.needs.map(s => String(s).toLowerCase()) : [];
    const aiSkin = typeof p.ai?.skin_type === 'string' ? p.ai.skin_type.toLowerCase() : null;
    ...
    const scored = doveProducts.filter(Boolean).map(prod => {
      ...
      const benefitsText = ((prod.description||'') + ' ' + (prod.benefits||[]).join(' ')).toLowerCase();

      // Besoins IA explicites → bonus fort
      if (aiNeeds.length) {
        aiNeeds.forEach(n => {
          if (benefitsText.includes(n)) { score += 30; reasons.push(`Besoin identifié: ${n}`); }
        });
      }

      // Type de peau IA → règles simples
      if (aiSkin) {
        if (/sec|très-sec/.test(aiSkin) && /hydrat|nourr/.test(benefitsText)) {
          score += 30; reasons.push('Peau sèche → hydratation');
        }
        if (/sensible/.test(aiSkin) && /apais|douceur|protection/.test(benefitsText)) {
          score += 20; reasons.push('Peau sensible → apaisement');
        }
        if (/gras|mixte/.test(aiSkin) && /léger|non-gras|éclat/.test(benefitsText)) {
          score += 15; reasons.push('Confort léger recommandé');
        }
      }

      // Règles environnementales (AI/local)
      if ((p.env?.uv_index ?? 0) > 6 && /bright|éclat/.test((prod.name + ' ' + (prod.description||'')).toLowerCase())) {
        score += 25; reasons.push('UV élevés aujourd’hui');
      }

      // Règles locales (fallback aux features locales si dispo)
      if ((p.skin_tone?.mst_bin ?? 4) <= 3 && /hydrat/i.test(benefitsText)) {
        score += 15; reasons.push('Peau potentiellement sèche');
      }
      if ((p.hair_features?.frizz_proxy ?? 0) > 0.5 && prod.category === 'soin-corps') {
        score += 10; reasons.push('Frisottis élevés');
      }

      // Bonus diversité
      score += ['soin-visage','soin-corps','gel-douche','soin-levres','soin-mains'].indexOf(prod.category) * 2;
      return { ...prod, matchScore: Math.min(100, score), _reasons: reasons };
    });
  ...
  const scoredProducts = doveProducts.filter(product => product && product.id).map(product => {
    ...
    if (product.environment && product.environment.includes(environment)) {
      score += 20;
    }
...
export const generateQRData = (userAnswers, selectedProducts) => {
  const data = {
    ...
    products: selectedProducts.filter(product => product && product.id).map(product => ({
      id: product.id,
...
export const calculateImpact = (products, frequency = 'monthly') => {
  ...
  const totalCO2 = safe.reduce((sum, product) => sum + (product.co2Saved || 0), 0) * multiplier;
  const totalPlastic = safe.reduce((sum, product) => sum + (product.plasticSaved || 0), 0) * multiplier;
...
export const formatImpactStats = (stats) => {
    return {
        ...
        money: {
            ...
            equivalence: stats.moneySaved ? `${Math.max(1, Math.round((stats.moneySaved || 0) / 4))} cafés` : '',
        },
        refills: {
