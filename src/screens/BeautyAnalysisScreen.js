...
export default function BeautyAnalysisScreen({ route, navigation }) {
	...
	useEffect(() => {
		...
		;(async () => {
			try {
				...
				if (Platform.OS === "web") {
					console.warn(
						"[AI] Web: appels directs API possibles erreurs CORS. Préférez iOS/Android.",
					)
				}
				console.log("[AI] Début analyse (Gemini seule)", {
					hasBase64: !!base64,
					coords,
				})
				...
				let out = null
				try {
					out = await analyzeWithGemini({
						base64,
						envSignals: env,
						products: doveProducts,
					})
					console.log("[AI] Analyse Gemini réussie")
				} catch (gerr) {
					console.error("[AI] Gemini KO", {
						code: gerr?.code,
					...
					setError(
						gerr?.code === 429
							...
							: "Analyse IA indisponible. Vérifiez la connexion et la clé.",
					)
					setProfile(null)
					setRecs([])
					return
				...
				const p = {
					...
					ai: {
						...
						needs: Array.isArray(out?.analysis?.needs)
							? out.analysis.needs
							: [],
						notes: Array.isArray(out?.analysis?.notes)
							? out.analysis.notes
							: [],
						...
						hair: out?.analysis?.hair
							? {
									...
									frizz:
										typeof out.analysis.hair.frizz ===
										"number"
											? out.analysis.hair.frizz
											: null,
									shine:
										typeof out.analysis.hair.shine ===
										"number"
											? out.analysis.hair.shine
											: null,
							  }
							: {
									type: "--",
				...
				try {
					const inferNeedsFromProfile = (pp) => {
						if (!pp) return []
						if (Array.isArray(pp.ai?.needs) && pp.ai.needs.length)
							return pp.ai.needs.slice(0, 3)
						const needs = []
						if ((pp.hair_features?.frizz_proxy ?? 0) > 0.5)
							needs.push("réparation")
						if ((pp.image_quality?.exposure ?? 0.5) < 0.35)
							needs.push("éclat")
						if ((pp.skin_tone?.mst_bin ?? 4) <= 3)
							needs.push("hydratation")
						return Array.from(
							new Set([...(pp.needs || []), ...needs]),
						).slice(0, 3)
					}
					const autoAnswers = {
						_beautyProfile: p,
						_aiRecs: Array.isArray(mappedRecs)
							? mappedRecs.slice(0, 8)
							: [],
						step1:
							p?.ai?.skin_type ||
							(p?.skin_tone
								? p.skin_tone.mst_bin <= 3
									? "sec"
									: p.skin_tone.mst_bin >= 7
									? "gras"
									: "normal"
								: "normal"),
						step2:
					...
					console.log(
						"[AI] Bilan IA sauvegardé pour consultation ultérieure",
					)
				} catch (persistErr) {
					console.warn(
						"[AI] Impossible de sauvegarder le bilan IA",
						persistErr,
					)
				}
			} catch (e) {
	...
	const inferNeedsFromProfile = (p) => {
		...
		if (Array.isArray(p.ai?.needs) && p.ai.needs.length)
			return p.ai.needs.slice(0, 3)
		const needs = []
	...
	const handleContinue = async () => {
		...
		const answers = {
			...
			step1:
				profile?.ai?.skin_type ||
				(profile?.skin_tone
					? profile.skin_tone.mst_bin <= 3
						? "sec"
						: profile.skin_tone.mst_bin >= 7
						? "gras"
						: "normal"
					: "normal"),
			step2:
				(profile?.env?.uv_index ?? 4) > 6
					? "élevée"
					: (profile?.env?.humidity ?? 50) > 70
					? "moyenne"
					: "faible",
		...
		console.log(
			"[AI] Navigation vers ProductMatching avec réponses",
			answers,
		)
	...
	const hairLabel = useMemo(() => {
		...
		const parts = []
		if (t) parts.push(t)
		if (typeof f.frizz_proxy === "number")
			parts.push(`frisottis ${(f.frizz_proxy * 100).toFixed(0)}%`)
		if (typeof f.shine_proxy === "number")
			parts.push(`brillance ${(f.shine_proxy * 100).toFixed(0)}%`)
		return parts.join(" • ") || "—"
	}, [profile])
			...
			<SafeAreaView style={styles.safeArea}>
				<View style={[styles.container, styles.center]}>
					...
					>
						...
						>
							...
							<View style={{ marginLeft: spacing.sm }}>
								...
								>
									Scannez votre peau pour des recommandations
									personnalisées
								</Text>
							</View>
						</View>
					...
					<Text style={styles.info}>
						Aucune photo. Reprenez une photo.
					</Text>
					<AppButton
						label="Ouvrir la caméra"
						onPress={() => navigation.replace("CameraCapture")}
						...
						<View style={styles.card}>
							...
							<View style={styles.rowBetween}>
								...
								<View style={styles.statBox}>
									...
									<Text style={styles.statValue}>
										{(
											inferNeedsFromProfile(profile) || []
										).join(" • ") || "—"}
									</Text>
								</View>
							</View>
							{Array.isArray(profile?.ai?.notes) &&
							profile.ai.notes.length ? (
					...
					{profile?.env ? (
						<View style={styles.card}>
							...
							<View style={styles.envRow}>
								<EnvPill
									label="UV"
									value={String(profile.env.uv_index)}
								/>
								<EnvPill
									label="Humidité"
									value={`${Math.round(
										profile.env.humidity,
									)}%`}
								/>
								<EnvPill
									label="PM2.5"
