...
const NAV_ROUTES = ["Dashboard", "QRCode", "RefillMap", "Profile"]
...
export default function FooterNavigation({ navigationRef, currentRoute }) {
	...
	const handleNavigate = (routeName, extraParams) => {
		if (!navigationRef?.navigate) return
		if (routeName === currentRoute) return

		const currentIndex = NAV_ROUTES.indexOf(currentRoute)
		const targetIndex = NAV_ROUTES.indexOf(routeName)
		...
		<View pointerEvents="box-none" style={styles.wrap}>
			...
			>
				...
				<DockButton
					...
					onPress={() => handleNavigate("RefillMap")}
